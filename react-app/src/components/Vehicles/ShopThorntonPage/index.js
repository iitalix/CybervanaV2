...
export default function ShopThorntonPage() {

    const dispatch = useDispatch();
    const user = useSelector((state) => state.session.user);
    const allVehicles = useSelector((state) => state.vehicles.allVehicles);
    const arrVehicles = Object.values(allVehicles).filter((vehicle) => vehicle.make === "Thornton");
    ...
    return (
        <div className="parent-container">
            <h1>Shop Thornton's Most Popular Vehicles</h1>
