...
export default function ShopMizutaniPage() {

    const dispatch = useDispatch();
    const user = useSelector((state) => state.session.user);
    const allVehicles = useSelector((state) => state.vehicles.allVehicles);
    const arrVehicles = Object.values(allVehicles).filter((vehicle) => vehicle.make === "Mizutani");
    ...
    return (
        <div className="parent-container">
            <h1>Shop Mizutani's Most Popular Vehicles</h1>
