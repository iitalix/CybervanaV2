...
const actionGetVehicleDetails = (vehicle) => ({
  type: GET_VEHICLE_DETAILS,
  vehicle,
});
const actionDeleteVehicle = (id) => ({type: DELETE_VEHICLE, id});
const actionGetOwnerVehicles = (vehicles) => ({
  type: GET_OWNER_VEHICLES,
  vehicles,
});
...
export default function vehicleReducer(state = initialState, action) {
  ...
  switch (action.type) {
    case GET_ALL_VEHICLES:
      ...
      action.vehicles.forEach(
        (vehicle) => (newState.allVehicles[vehicle.id] = vehicle)
      );
      return newState;
    ...
    case GET_OWNER_VEHICLES:
      ...
      action.vehicles.forEach(
        (vehicle) => (newState.ownerVehicles[vehicle.id] = vehicle)
      );
      return newState;
